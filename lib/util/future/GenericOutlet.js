    ...
    var executeSafe = function (messageObj) {
        ...
        try {
            if (messageObj.meta.command == "getIdentity") {
                ...
                if (global_swarmSystem_config.Core.checkAccess) {
                    checkAccess("swarm", messageObj.meta.swarmingName, "ctor", messageObj.meta.ctor, "execution", messageObj.meta.userId ? messageObj.meta.userId : "NO_USER", function (err, hasRights) {
                        if (err) {
                            logger.logError("Failing to start swarm: " + messageObj.meta.swarmingName + " ctor: " + messageObj.meta.ctor, err);
                        } else if (!hasRights) {
                            logger.logError("User " + getCurrentUser() + " does not have access rights to resource 'swarm/" + messageObj.meta.swarmingName + "/ctor/" + messageObj.meta.ctor, "' for execution")
                        } else {
                            runExecutionContext();
                        }
                    })
                ...
                function runExecutionContext(){
                    try {
                        ...
                        if (throttler.accept(function () {
                                ...
                                if(global_swarmSystem_config.Core.performSwarmAnalytics){
                                    try{
                                        performAnalytics(messageObj.meta.userId?messageObj.meta.userId:"NO_USER",messageObj.meta.swarmingName,messageObj.meta.ctor,swarming.meta,args)
                                    }catch(err)
                                    {
                                        logger.logError("Failed to perform analytics.",err);
                                    }
                                }
                            }));
                    } catch (err) {
